...
class FontSampler:
    def __init__(self, font_dict, page_size) -> None:
        ...
        self.font_statics = pd.read_csv(font_statics)
...
def render_comictext(comic_sampler_list, img_dir, label_dir=None, render_num=700, save_dir=None, save_prefix=None):
    ...
    for ii in tqdm(range(render_num)):
        if statistics is not None:
            imgname = statistics.loc[ii]['name']
        else:
            imgname = imglist[ii]
...
if __name__ == '__main__':
    ...
    # canvas, canvas_msk = cts.drawtext_one_page()
